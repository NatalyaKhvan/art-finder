  ...
  try {
    const response = await fetch(
      `${apiUrl}/search?q=${encodeURIComponent(
        query
      )}&query[term][is_public_domain]=true&fields=id,title,image_id,artist_title&limit=${limit}`
    );
...
function displayArtworks(artworks) {
  ...
  artworks.forEach((artwork) => {
    ...
    let altText;
    if (artwork.title) {
      altText = `${artwork.title} by ${
        artwork.artist_title || "Unknown Artist"
      }`;
    } else {
      altText = "Artwork with no title";
    }
  ...
  try {
    ...
    const modalContent = `
        ...
        <p><strong>Medium:</strong> ${
          artwork.medium_display || "Unknown Medium"
        }</p>
        ...
        <img src="https://www.artic.edu/iiif/2/${
          artwork.image_id
        }/full/800,/0/default.jpg" alt="${artwork.title}">
      `;
